...
下線付きの色文字リンク(.text-link-btn風)に変更したところ、今度は「下線リンクのスタイル自体が
前々からスタイリッシュじゃなくて好きじゃない」と指摘があった。mcp__visualize（Imagine）で
4案（矢印+下線なし文字／輪郭だけのゴーストボタン／区切り線埋め込みラベル／丸アイコン+ラベル）を
アプリの実際の配色で並べて提示し、ユーザーが**B案（輪郭だけのゴーストボタン、幅いっぱい・
角丸pill・var(--text-dim)文字）**を選択。最終的にこの見た目で実装した。「編集する」「＋ 新しいプランを作る」等と同じ
